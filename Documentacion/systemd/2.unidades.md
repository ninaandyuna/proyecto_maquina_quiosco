...
para no tocar el fichero de unidad original



Por otro lado, podemos copiar el archivo de la antigua unidad desde /usr/lib/systemd/system/ a /etc/systemd/system/ y realizar los cambios allí. Un archivo de unidad ubicado en /etc/systemd/system/ siempre tiene preferencia sobre la misma unidad localizada en /usr/lib/systemd/system/
...
Esto abre el fichero _/etc/systemd/system/unidad.d/override.conf_ en un editor de texto (creándolo si es necesario) y vuelve a cargar automáticamente la unidad cuando hemos terminado de editar.

Por ejemplo, si deseamos simplemente agregar una dependencia adicional a una unidad, podemos crear el siguiente archivo _/etc/systemd/system/unidad.d/nueva\_dependencia.conf_:
...
Siguiendo otro ejemplo, con el fin de reemplazar la directiva _ExecStart_ para una unidad que no es del tipo _oneshot_, crearemos el siguiente fichero _/etc/systemd/system/unidad.d/nuevo\_exec.conf_:
